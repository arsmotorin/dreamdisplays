...
import com.dreamdisplays.displays.managers.SettingsManager.updateSettings
...
class DisplayScreen(
    ...
    var brightness: Float = 0f
        set(value) {
            ...
            if (mediaPlayer != null) MediaPlayerControls.setBrightness(field.toDouble())
            // Save settings
    ...
    var quality: String = "720"
        set(value) {
            field = value
            if (mediaPlayer != null) MediaPlayerControls.setQuality(mediaPlayer!!, value)
            // reloadTexture();
            // Save settings
    ...
    init {
        ...
        volume = settings.volume.toDouble()
    ...
    fun loadVideo(videoUrl: String, lang: String?) {
        ...
        val timecodeSeconds = Utils.extractTimecode(videoUrl)

        // Load the video URL and language into the display
        ...
        waitForMFInit {
            ...
            if (shouldBePaused) {
                this.paused = true
                if (mediaPlayer != null) MediaPlayerControls.pause(mediaPlayer!!)
            }
        }
    ...
    // Updates the display data based on a DisplayInfoPacket
    ...
    // Updates the display data based on a SyncPacket
    ...
    // Checks if a given BlockPos is within the display boundaries
    fun isInDisplay(pos: BlockPos): Boolean {
        var maxX = x
        var maxY = y
        var maxZ = z
    ...
    // Calculates the distance from a given BlockPos to the closest point on the display
    fun getDistanceToDisplay(pos: BlockPos): Double {
        var maxX = x
        var maxY = y
        var maxZ = z
    ...
    // Returns display position as BlockPos
    fun getPos(): BlockPos {
        if (blockPos == null) {
    ...
    // Returns display facing direction
    ...
    fun startVideo() {
        if (mediaPlayer != null) {
            videoStarted = true
            if (paused) {
                MediaPlayerControls.pause(mediaPlayer!!)
            } else {
                MediaPlayerControls.play(mediaPlayer!!)
                paused = false
            }
            restoreSavedTime()
        }
    ...
    fun seekVideoRelative(seconds: Long) {
        if (mediaPlayer != null) MediaPlayerControls.seekRelative(mediaPlayer!!, seconds.toDouble())
    }

    // Absolute (cinema) seek video: moves to a specific second
    fun seekVideoTo(nanos: Long) {
        if (mediaPlayer != null) MediaPlayerControls.seekTo(mediaPlayer!!, nanos, false)
    }
    ...
    fun unregister() {
        if (mediaPlayer != null) MediaPlayerControls.stop(mediaPlayer!!)
        ...
        if (minecraft.screen is ConfigurationManager) {
            ...
            if (displayConfScreen.display === this) displayConfScreen.onClose()
        }
    }
    ...
    // Creates a new texture for the display based on its dimensions and quality
    fun createTexture() {
        val qualityInt = parseInt(this.quality.replace("p", ""))
        textureWidth = ((width / height.toDouble()) * qualityInt).toInt()
        textureHeight = qualityInt
    ...
    fun restoreSavedTime() {
        if (savedTimeNanos > 0 &&
            ...
            MediaPlayerControls.seekToFast(mediaPlayer!!, savedTimeNanos)
        }
    }
