package com.dreamdisplays.displays.mediaplayer.controls
...
import com.dreamdisplays.displays.mediaplayer.utils.GStreamer
...
object MediaPlayerControls {
    ...
    fun seekTo(mp: MediaPlayer, nanos: Long, b: Boolean) {
        safeExecute(mp) {
            ...
            if (mp.videoPipeline != null) mp.videoPipeline!!.state
            mp.audioPipeline!!.play()
            if (mp.videoPipeline != null && !mp.display.getPaused()) mp.videoPipeline!!.play()

            if (b) mp.display.afterSeek()
        }
    }
    ...
    fun setVolume(mp: MediaPlayer, volume: Double) {
        ...
        mp.currentVolume = userVolume * lastAttenuation
        safeExecute(mp) { applyVolume(mp) }
    }
    ...
    private fun applyVolume(mp: MediaPlayer) {
        AudioPipeline.Companion.applyVolume(mp.audioPipeline, mp.currentVolume)
    }
    ...
    private fun changeQuality(mp: MediaPlayer, desired: String) {
        ...
        try {
            ...
            Minecraft.getInstance().execute { mp.display.reloadTexture() }
            ...
            val newVid = VideoPipeline.Companion.build(best.get().url!!) { sink ->
                mp.bufferPreparator.configureVideoSink(sink)
            }
            ...
            mp.currentVideoUrl = best.get().url
    ...
    fun safeExecute(mp: MediaPlayer, r: Runnable) {
        if (!mp.gstExecutor.isShutdown) {
            try {
                mp.gstExecutor.submit(r)
